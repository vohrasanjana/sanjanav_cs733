...
func TestRPCMain(t *testing.T) {
	...
	conf := make([]*raft.Config,(NUMRAFTS+1))

	raft.InitSM(NUMRAFTS)

	for i:=1;i<=NUMRAFTS;i++ {
		conf[i] = &raft.Config{cluster,i,elec_cand,elec_foll,hb_t,}

	}
	addr = []string{"localhost:8080","localhost:8081","localhost:8082","localhost:8083","localhost:8084"}

	rfNodes = make([]*raft.RaftNode, len(clconfig.Peers))
	for i:=1;i<=NUMRAFTS;i++ {
		
		rfNodes[i-1] = serverMain(i,addr,conf[i])
		rfNodes[i-1].DoActions([]raft.Action{raft.Alarm{rfNodes[i-1].Conf.ElecTimeFoll}})
	}	 
...
func TestRPC_BasicSequential(t *testing.T) {
	...
	m, err = cl.read("cs733net")
	expect(t, m, &Msg{Kind: 'F'}, "file not found", err)
	fmt.Println("SUCCESSFUL - BASIC SEQUENTIAL")
}
...
func TestRPC_Binary(t *testing.T) {
	...
	expect(t, m, &Msg{Kind: 'C', Contents: []byte(data)}, "read my write", err)
	fmt.Println("SUCCESSFUL - BINARY CONTENTS")
...
func TestRPC_Chunks(t *testing.T) {
	...
	fmt.Println("SUCCESSFUL - CHUNKS OF COMMANDS")
}
...
func TestRPC_Batch(t *testing.T) {
	...
	fmt.Println("SUCCESSFUL - BATCH COMMANDS")
}
...
func TestRPC_BasicTimer(t *testing.T) {
	...
	fmt.Println("SUCCESSFUL - TIMER TESTING OF FILES")
...
func TestRPC_ConcurrentWrites(t *testing.T) {
	...
	fmt.Println("SUCCESSFUL - CONCURRENT WRITES")
}
